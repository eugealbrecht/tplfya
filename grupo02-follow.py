...
class Gramatica():
    ...
    def __init__(self, gramatica):
        ...
        self.diccionario = dict.fromkeys(self.antecedentes)
        ...
        for i in self.gramatica:
            ...
            if (self.diccionario[i[0]] == None):  # Si el diccionario cuya key es i[0] está vacio:
                ...
                self.diccionario[i[0]] = [d[0]]  # agregamos el consecuente de la "key" directamente
            else:
                self.diccionario[i[0]].append(d[0])
        print('FIRST')
        print(self.first)
        print('FOLLOW')
        print(self.follows)
    ...
    def parse(self, cadena):
        ...
        stack = []
        stack.append('$')
        stack.append(self.no_terminales[0])
        input_le = len(cadena)
        index = 0
        while len(stack) > 0:
            top = stack[- 1]
            #print("Top =>", top)
            current_input = cadena[index]
            ...
            if top == current_input:
                ...
                print(key)
                if key not in TablaGenerada:
                    flag = 1
                    break
                value = TablaGenerada[key]
                if value != 'lambda':
                    value = value[::-1]
                    value = list(value)
                    stack.pop()
